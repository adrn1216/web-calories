...
export default function Home() {
  ...
  return <main className="mx-auto flex h-dvh max-w-lg flex-col overflow-hidden bg-white">
    ...
    <LastWeekSummarySheet open={lastWeekOpen} summary={previousWeek} weight={weeklyWeights[previousWeekKey] ?? 0} weights={weeklyWeights} onClose={() => setLastWeekOpen(false)} onSave={(calories, weight) => { setWeekAdjustments((values) => ({ ...values, [previousWeekKey]: calories })); setWeeklyWeights((values) => ({ ...values, [previousWeekKey]: weight })); setLastWeekOpen(false); notify("Last week summary saved"); }} />
